!
module coef_mp
  use gather_scatter_mp, only : gs_dp_t, gs_sp_t
  ...
  use math, only : rone, invcol1, addcol3, subcol3, copy, &
       chsign, rzero, rzero_sp, invers2, glsum, NEKO_EPS, &
       rone_sp, invcol1_sp, copy_sp, addcol3_sp, subcol3_sp, &
       chsign_sp, invers2_sp, glsum_sp
  ...
  use mxm_wrapper, only : mxm, mxm_sp
  use device
  use, intrinsic :: iso_c_binding
  implicit none
  private

  ! Single precision coefficient type
    !> Coefficients defined on a given (mesh, \f$ X_h \f$) tuple.
  !! Arrays use indices (i,j,k,e): element e, local coordinate (i,j,k).
  type, public :: coef_sp_t
     !> Geometric factors \f$ G_{11} \f$
     real(kind=sp), allocatable :: G11(:,:,:,:)
     !> Geometric factors \f$ G_{22} \f$
     real(kind=sp), allocatable :: G22(:,:,:,:)
     !> Geometric factors \f$ G_{33} \f$
     real(kind=sp), allocatable :: G33(:,:,:,:)
   !   !> Geometric factors \f$ G_{12} \f$
   !   real(kind=sp), allocatable :: G12(:,:,:,:)
   !   !> Geometric factors \f$ G_{13} \f$
   !   real(kind=sp), allocatable :: G13(:,:,:,:)
   !   !> Geometric factors \f$ G_{23} \f$
   !   real(kind=sp), allocatable :: G23(:,:,:,:)

     real(kind=sp), allocatable :: mult(:,:,:,:) !< Multiplicity
   ...
   !   real(kind=sp), allocatable :: dxdr(:,:,:,:), dydr(:,:,:,:), dzdr(:,:,:,:)
   !   real(kind=sp), allocatable :: dxds(:,:,:,:), dyds(:,:,:,:), dzds(:,:,:,:)
   !   real(kind=sp), allocatable :: dxdt(:,:,:,:), dydt(:,:,:,:), dzdt(:,:,:,:)
   !   !> \f$ dr/dx, dr/dy, dr/dz \f$
   !   !! \f$ ds/dx, ds/dy, ds/dz \f$
   !   !! \f$ dt/dx, dt/dy, dt/dz \f$
   !   real(kind=sp), allocatable :: drdx(:,:,:,:), drdy(:,:,:,:), drdz(:,:,:,:)
   !   real(kind=sp), allocatable :: dsdx(:,:,:,:), dsdy(:,:,:,:), dsdz(:,:,:,:)
   !   real(kind=sp), allocatable :: dtdx(:,:,:,:), dtdy(:,:,:,:), dtdz(:,:,:,:)

   !   real(kind=sp), allocatable :: h1(:,:,:,:) !< Stiffness scaling
   !   real(kind=sp), allocatable :: h2(:,:,:,:) !< Mass scaling
   !   logical :: ifh2 !< True if h2 .ne. 0

   !   real(kind=sp), allocatable :: jac(:,:,:,:) !< Jacobian
   !   real(kind=sp), allocatable :: jacinv(:,:,:,:) !< Inverted Jacobian
   !   real(kind=sp), allocatable :: B(:,:,:,:) !< Mass matrix/volume matrix
   !   real(kind=sp), allocatable :: Binv(:,:,:,:) !< Inverted Mass matrix/volume matrix

   !   real(kind=sp), allocatable :: area(:,:,:,:) !< Facet area
   !   real(kind=sp), allocatable :: nx(:,:,:,:)   !< x-direction of facet normal
   !   real(kind=sp), allocatable :: ny(:,:,:,:)   !< y-direction of facet normal
   !   real(kind=sp), allocatable :: nz(:,:,:,:)   !< z-direction of facet normal
   !   !> Pointers to main fields

     real(kind=sp) :: volume

   !   type(space_sp_t), pointer :: Xh => null()
   !   type(mesh_t), pointer :: msh => null()
   !   type(dofmap_sp_t), pointer :: dof => null()
   !   type(gs_sp_t), pointer :: gs_h=> null()
   ...
   ! contains
   !   procedure, private, pass(this) :: init_empty => coef_init_empty_sp
   !   procedure, private, pass(this) :: init_all => coef_init_all_sp
   !   procedure, pass(this) :: free => coef_free_sp
   !   procedure, pass(this) :: get_normal => coef_get_normal_sp
   !   procedure, pass(this) :: get_area => coef_get_area_sp
  ...
  !> double precision
  !
  type, public :: coef_dp_t
     !> Geometric factors \f$ G_{11} \f$
     real(kind=dp), allocatable :: G11(:,:,:,:)
     !> Geometric factors \f$ G_{22} \f$
     real(kind=dp), allocatable :: G22(:,:,:,:)
     !> Geometric factors \f$ G_{33} \f$
     real(kind=dp), allocatable :: G33(:,:,:,:)
     !> Geometric factors \f$ G_{12} \f$
     real(kind=dp), allocatable :: G12(:,:,:,:)
     !> Geometric factors \f$ G_{13} \f$
     real(kind=dp), allocatable :: G13(:,:,:,:)
     !> Geometric factors \f$ G_{23} \f$
     real(kind=dp), allocatable :: G23(:,:,:,:)
     ...
     real(kind=dp), allocatable :: h1(:,:,:,:) !< Stiffness scaling
     real(kind=dp), allocatable :: h2(:,:,:,:) !< Mass scaling
     logical :: ifh2 !< True if h2 .ne. 0
     ...
     type(space_dp_t), pointer :: Xh => null()
     type(mesh_t), pointer :: msh => null()
     type(dofmap_dp_t), pointer :: dof => null()
     type(gs_dp_t), pointer :: gs_h=> null()
   ...
   contains
     procedure, private, pass(this) :: init_empty => coef_init_empty_dp
     procedure, private, pass(this) :: init_all => coef_init_all_dp
     procedure, pass(this) :: free => coef_free_dp
     procedure, pass(this) :: get_normal => coef_get_normal_dp
     procedure, pass(this) :: get_area => coef_get_area_dp
     generic :: init => init_empty, init_all
  end type coef_dp_t
...
!   subroutine coef_init_empty_sp(this, Xh, msh)
!     class(coef_sp_t), intent(inout) :: this
!     type(space_sp_t), intent(inout), target :: Xh
...
!   end subroutine coef_init_empty_sp

!   !> Initialize coefficients
!   subroutine coef_init_all_sp(this, gs_h)
!     class(coef_sp_t), intent(inout) :: this
!     type(gs_sp_t), intent(inout), target :: gs_h
!     integer :: n, m
!     call this%free()
...
!     call coef_generate_dxyzdrst_sp(this)

!     call coef_generate_geo_sp(this)

!     call coef_generate_area_and_normal_sp(this)

!     call coef_generate_mass_sp(this)
...
!       !                     DEVICE_TO_HOST, sync=.false.)
!     else
!        call rone_sp(this%h1,n)
!        call rone_sp(this%h2,n)
!     end if
...
!     else
!        call invcol1_sp(this%mult, n)
!     end if

!   end subroutine coef_init_all_sp

!   !> Deallocate coefficients
!   subroutine coef_free_sp(this)
!     class(coef_sp_t), intent(inout) :: this
...
!   end subroutine coef_free_sp

!   subroutine coef_generate_dxyzdrst_sp(c)
!     type(coef_sp_t), intent(inout) :: c
...
!       else
!          do e = 1, c%msh%nelv
!             call mxm_sp(dx, lx, x(1,1,1,e), lx, dxdr(1,1,1,e), lyz)
!             call mxm_sp(dy, lx, x(1,1,1,e), lx, dydr(1,1,1,e), lyz)
!             call mxm_sp(dz, lx, x(1,1,1,e), lx, dzdr(1,1,1,e), lyz)

!             do i = 1, lz
!                call mxm_sp(x(1,1,i,e), lx, dyt, ly, dxds(1,1,i,e), ly)
!                call mxm_sp(y(1,1,i,e), lx, dyt, ly, dyds(1,1,i,e), ly)
!                call mxm_sp(z(1,1,i,e), lx, dyt, ly, dzds(1,1,i,e), ly)
!             end do

!             ! We actually take 2d into account, wow, need to do that for the rest.
!             if(c%msh%gdim .eq. 3) then
!                call mxm_sp(x(1,1,1,e), lx, dzt, lz, dxdt(1,1,1,e), lz)
!                call mxm_sp(y(1,1,1,e), lx, dzt, lz, dydt(1,1,1,e), lz)
!                call mxm_sp(z(1,1,1,e), lx, dzt, lz, dzdt(1,1,1,e), lz)
!             else
!                call rzero_sp(dxdt(1,1,1,e), lxy)
!                call rzero_sp(dydt(1,1,1,e), lxy)
!                call rone_sp(dzdt(1,1,1,e), lxy)
!             end if
!          end do

!          if (c%msh%gdim .eq. 2) then
!             call rzero_sp   (jac, ntot)
!             call addcol3_sp (jac, dxdr, dyds, ntot)
!             call subcol3_sp (jac, dxds, dydr, ntot)
!             call copy_sp    (drdx, dyds, ntot)
!             call copy_sp    (drdy, dxds, ntot)
!             call chsign_sp  (drdy, ntot)
!             call copy_sp    (dsdx, dydr, ntot)
!             call chsign_sp  (dsdx, ntot)
!             call copy_sp    (dsdy, dxdr, ntot)
!             call rzero_sp   (drdz, ntot)
!             call rzero_sp   (dsdz, ntot)
!             call rone_sp    (dtdz, ntot)
!          else

!             do i = 1, ntot
!                c%jac(i, 1, 1, 1) = 0.0_sp
!             end do
...
!          end if
!          call invers2_sp(jacinv, jac, ntot) ! jac, jacinv => wrong
!       end if
!     end associate

!   end subroutine coef_generate_dxyzdrst_sp

!   !> Generate geometric data for the given mesh
!   !! @note Current implementation assumes regular shaped hex elements
!   subroutine coef_generate_geo_sp(c)
!     type(coef_sp_t), intent(inout) :: c
...
!   end subroutine coef_generate_geo_sp
...
!   subroutine coef_generate_mass_sp(c)
!     type(coef_sp_t), intent(inout) :: c
!     integer :: e, i, lxyz, ntot
...
!     else
!        c%volume = glsum_sp(c%B, ntot)
!     end if

!   end subroutine coef_generate_mass_sp

!   pure function coef_get_normal_sp(this, i, j, k, e, facet) result(normal)
!     class(coef_sp_t), intent(in) :: this
!     integer, intent(in) :: i, j, k, e, facet
!     real(kind=sp) :: normal(3)
...
!     end select
!   end function coef_get_normal_sp

!   pure function coef_get_area_sp(this, i, j, k, e, facet) result(area)
!     class(coef_sp_t), intent(in) :: this
!     integer, intent(in) :: i, j, k, e, facet
!     real(kind=sp) :: area
...
!       end select
!   end function coef_get_area_sp


!   !> Generate facet area and surface normals
!   subroutine coef_generate_area_and_normal_sp(coef)
!     type(coef_sp_t), intent(inout) :: coef
!     real(kind=sp), allocatable :: a(:,:,:,:)
!     real(kind=sp), allocatable :: b(:,:,:,:)
!     real(kind=sp), allocatable :: c(:,:,:,:)
!     real(kind=sp), allocatable :: dot(:,:,:,:)
!     integer :: n, e, i, j, k, lx
!     real(kind=sp) :: weight, len
!     n = coef%dof%size()
!     lx = coef%Xh%lx
...
!   end subroutine coef_generate_area_and_normal_sp

  ! Double precision

  !> Initialize empty coefs for a space and a mesh
  subroutine coef_init_empty_dp(this, Xh, msh)
    class(coef_dp_t), intent(inout) :: this
    type(space_dp_t), intent(inout), target :: Xh
    type(mesh_t), intent(inout), target :: msh
    integer :: n
    call this%free()
    this%msh => msh
  ...
  end subroutine coef_init_empty_dp

  !> Initialize coefficients
  subroutine coef_init_all_dp(this, gs_h)
    class(coef_dp_t), intent(inout) :: this
    ...
    call coef_generate_dxyzdrst_dp(this)

    call coef_generate_geo_dp(this)

    call coef_generate_area_and_normal_dp(this)

    call coef_generate_mass_dp(this)
  ...
  end subroutine coef_init_all_dp

  !> Deallocate coefficients
  subroutine coef_free_dp(this)
    class(coef_dp_t), intent(inout) :: this
  ...
  end subroutine coef_free_dp

  subroutine coef_generate_dxyzdrst_dp(c)
    type(coef_dp_t), intent(inout) :: c
    integer :: e,i,lxy,lyz,ntot
  ...
  end subroutine coef_generate_dxyzdrst_dp

  !> Generate geometric data for the given mesh
  !! @note Current implementation assumes regular shaped hex elements
  subroutine coef_generate_geo_dp(c)
    type(coef_dp_t), intent(inout) :: c
    integer :: e, i, lxyz, ntot
  ...
  end subroutine coef_generate_geo_dp

  !> Generate mass matrix B for the given mesh and space
  !! @note This is also a stapleholder, we need to go through the coef class properly.
  subroutine coef_generate_mass_dp(c)
    type(coef_dp_t), intent(inout) :: c
    integer :: e, i, lxyz, ntot
  ...
  end subroutine coef_generate_mass_dp

  pure function coef_get_normal_dp(this, i, j, k, e, facet) result(normal)
    class(coef_dp_t), intent(in) :: this
    integer, intent(in) :: i, j, k, e, facet
  ...
  end function coef_get_normal_dp

  pure function coef_get_area_dp(this, i, j, k, e, facet) result(area)
    class(coef_dp_t), intent(in) :: this
    integer, intent(in) :: i, j, k, e, facet
    real(kind=dp) :: area
  ...
  end function coef_get_area_dp


  !> Generate facet area and surface normals
  subroutine coef_generate_area_and_normal_dp(coef)
  ...
  end subroutine coef_generate_area_and_normal_dp
