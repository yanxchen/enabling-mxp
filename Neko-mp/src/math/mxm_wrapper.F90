...
#ifdef HAVE_LIBXSMM
    call mxm_libxsmm_sp(a,n1,b,n2,c,n3)
#else
    call mxm_blas_sp(a,n1,b,n2,c,n3)
#endif

  end subroutine mxm_sp

  subroutine mxm_mp1(a,n1,b,n2,c,n3)
    integer, intent(in) :: n1, n2, n3
    real(kind=dp), intent(in) :: a(n1, n2) ! dp
    real(kind=dp), intent(in) :: b(n2, n3) ! dp
    real(kind=sp), intent(inout) :: c(n1, n3) ! sp

    real(kind=sp) :: a_sp(n1, n2) ! sp
    real(kind=sp) :: b_sp(n2, n3) ! sp

    a_sp = real(a, kind=sp)
    b_sp = real(b, kind=sp)

#ifdef HAVE_LIBXSMM
    call mxm_libxsmm_sp(a_sp,n1,b_sp,n2,c,n3)
#else
    call mxm_blas_sp(a_sp,n1,b_sp,n2,c,n3)
#endif

  end subroutine mxm_mp1
